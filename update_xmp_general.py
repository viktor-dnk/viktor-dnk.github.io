...
def process_article(md_path, new_date_str):
    ...
    for figure in figures:
        # ПРОВЕРКА НА АВТОРСКИЕ ПРАВА (ИНВЕРТИРОВАНА)
        has_copyright = (
        ...
        # Если есть метатеги копирайта - ПРОПУСКАЕМ это изображение
        if has_copyright:
            continue
        ...
        # 4. Формирование аргументов для ExifTool (Базовые XMP без копирайтов)
        args = [
            "-overwrite_original",
            "-charset", "exiftool=utf8",
            "-IPTC:all=",  # Полностью очищаем устаревший IPTC блок
            
            # === Базовые XMP (без указания авторства и прав) ===
            f"-XMP:Title={title}",
...
def main():
    ...
    if not articles:
        print("⚠️ Не найдено ни одной статьи для обработки.")
        return
    ...
    for md_path in articles:
        ...
        if count > 0:
            print(f"✅ Обновлено базовых изображений (без копирайтов): {count}")
            total_updated_images += count
            updated_articles_count += 1
        else:
            print("ℹ️ Изображений без копирайтов не найдено (или все уже защищены).")
